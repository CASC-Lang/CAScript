...
describe("evaluation error test", () => {
    it("invalid number format I", () => {
        ...
        expect(diagnostics[0].message).to.equal("ERROR: Invalid number format: '1..'");
    });
    ...
    it("cannot negate bool (-)", () => {
        ...
        expect(diagnostics.length).to.equal(1);
        expect(diagnostics[0].message).to.equal("ERROR: Cannot apply operator '-' on type 'bool'");
    });
	...
	it("bool cannot add with number", () => {
		...
		expect(diagnostics.length).to.equal(1);
        expect(diagnostics[0].message).to.equal("ERROR: Cannot apply operator '+' on type 'bool' and type 'number'")
	});
});
